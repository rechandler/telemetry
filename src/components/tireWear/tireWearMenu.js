...
export default () => {
    const [launchEnabled, setLaunchEnabled] = useState(store.get('tireWearWidget.launchEnabled') || false)
    const [positionEnabled, setPositionEnabled] = useState(store.get('tireWearWidget.positionEnabled') || false)

    
    const updateLaunchEnabled = value => {
        store.set('tireWearWidget.launchEnabled', value)
        setLaunchEnabled(value)
    }

    const updatePositionEnabled = value => {
        store.set('tireWearWidget.positionEnabled', value)
        setPositionEnabled(value)
    }

    const launchTireWearWidget = () => ipcRenderer.send('tireWearLaunch')
    ...
    return (
        <div className="py-6">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h1 className="text-2xl font-semibold text-gray-900">TireWear Widget Options</h1>
            </div>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
                ...
                <div className="py-4">
                    ...
                    <button
                        ...
                        onClick={launchTireWearWidget}
                    >
                        Launch TireWear Widget
                    </button>
                </div>
            </div>
        </div>
